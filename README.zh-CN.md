...
**安全性**：这个包已用 65 个杀毒引擎扫过，**0 报毒**（含阿里、安天等国产引擎）：

<https://www.virustotal.com/gui/file/d7f0805792ec6a26b17ca1bbdf88df88042da3946afdf7bf8ba9662a2802923a>

> 注意 VirusTotal 上的哈希是**压缩包**的哈希，和 `cxmon.exe` 的哈希不是
> 同一个东西（exe 的 SHA256 见发布页和包内 `SHA256.txt`）。VT 会解包扫里面的
> exe，所以结论对 exe 同样成立。

> ⚠️ **exe 未做数字签名**（代码签名证书要每年上千元 + 实名验证，对免费工具不值得），
> 而打包成 exe 的程序里含一个 Python 解释器，**个别本地杀软仍可能误报** ——
> VirusTotal 的引擎和同学机器上装的软件判断标准不完全一样。
> 如果被拦截，把文件夹加入白名单即可；不放心就别用。
